async function loadEmployees() {
  ...
  data.forEach(emp => {
    ...
    card.className = "card";
    card.innerHTML = `
      <div class="profile-icon">${emp.name[0]}</div>
      ...
      <div class="info-row">
        ...
        <input type="text" id="designation-${emp._id}" value="${emp.designation}" disabled>
      </div>
      <div class="info-row">
...
function editEmployee(id) {
  ...
  document.querySelector(`.card-buttons button.save-btn[onclick="saveEmployee('${id}')"]`).disabled = false;
}
